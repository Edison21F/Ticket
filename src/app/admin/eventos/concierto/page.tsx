...
const MOCK_SEATS: Seat[] = Array.from({ length: 100 }, (_, i) => ({
  id: `seat-${i}`,
  number: i + 1,
...
const StadiumLayout: React.FC<Props> = ({ 
  isAdmin = false, 
  onSeatStatusChange,
  onSeatDelete,
  onSeatPriceChange 
            ...
            {getSectionSeats(selectedSection).map((seat) => (
              <div key={seat.id} className="relative group">
                <button
                  ...
                  onClick={() => seat.status === 'available' && !isAdmin && setSelectedSeat(seat)}
            ...
            <div className="flex gap-4 mt-8">
              ...
              <button 
                className="flex-1 px-4 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700
                  transition-colors duration-200"
                onClick={() => {
                  handleStatusChange(selectedSeat.id, 'reserved');
